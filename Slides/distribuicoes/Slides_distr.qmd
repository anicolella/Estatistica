...
A variável aleatória $X$ tem distribuição normal com Média $\mu$ e Variância $\sigma^{2}$, onde $-\infty<\mu<+\infty$ e $0< \sigma^{2}< \infty$. Sua densidade é dada por:
...
Assim, essa variável aleatória Z, possui a seguinte f.d.p.: $$\begin{array}{ccc}
...
a.  $$\begin{array}{ccc}
    ...
    \end{array}$$ Portanto, a probabilidade é de $50\%$
:::
...
b.  $$\begin{array}{ccc}
    ...
    \end{array}$$ Portanto, a probabilidade é de $9,1\%$

c.  $$\begin{array}{ccc}
    P(X>20000)=P( Z > \frac{20000-10000}{15000})
    \end{array}$$ $$\begin{array}{ccc}
    = P(Z>6,67) \cong 0
    \end{array}$$ Portanto, a probabilidade é praticamente zero.
:::
...
possui distribuição F com $v_1$ e $v_2$ graus de liberdade, tal que $W \sim F(v_1,v_2)$. Dessa forma, uma variável aleatória contínua com distribuição F tem a seguinte f.d.p.:
