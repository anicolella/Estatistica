...
```{r, results='hide'}
names(data_feminic22)<-c("estado", "regiao", "homic_abs", "feminic_abs") 
data_feminic22
```
...
```{r}
df_feminic22 <- read.csv("../dados/dados_feminic.csv", head=TRUE, sep=";")

df_t_feminic22 <- read.csv("../dados/dados_tent_feminic.csv", head=TRUE, sep=";")
...
```{r,eval=F}
write.table(df_feminic22, "../dados/export_feminic.csv", sep=";")
```
...
```{r,eval=F}
library(foreign)
write.dta(df_feminic22, "../dados/export_feminic.dta")
```
...
write.table(final_fem_22, "../dados/final_fem_22.csv", sep=";")
```
