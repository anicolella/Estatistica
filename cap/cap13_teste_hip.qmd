...
1. Verificar se um determinado medicamento não tem efeito sobre a mortalidade causada por um determinado virus ou se possui efeito. 

2. Verificar se a quantidade de gordura anunciado pelo fabricante de um produto realmente está correta ou é maior. 

3. Se a afirmativa de um canditado de que possui a maioria dos votos é verdadeira ou é menor. 

4. Verificar se as rendas entre duas comunidades são as mesmas para podermos lançar uma política de apoio.

5. Verifcar se uma política do aumento do recurso as empresas não afeta falência ou se tem efeito.
...
$$P \bigg(\bar{X}\geq 700|H_0 \Rightarrow \bar{X} \sim N \bigg(600,\frac{100^2}{25}\bigg)\bigg)$$

$$P \bigg(Z_{\bar{X}}\geq \frac{700-600}{100/5} \Bigg|H_0 \bigg)$$

$$P \bigg(Z_{\bar{X}} \geq  \frac{100}{20} \bigg)=P(Z_{\bar{X}} \geq 5)=0$$
...
1. EI $(\alpha)$- Dizer que a renda é maior que 600, quando na realizadade ela é de 600.

2. EII $(\beta)$- Dizer que a renda é de 600 quando na realidade ela é maior do que 600. 
...
Vamos retomar o nosso exemplo onde a ONG afirma que uma comunidade tem renda de R\$600 e portanto deveria estar inclusa no programa de governo. O governo contexta. Foi retirada uma amostra de $n=25$ e a distribuição de $\bar{X}$ sob $H_0$ será $N \bigg(600,\frac{100^2}{25} \bigg)$ e a hipótese a ser testada será:
...
### Os Cinco passos para a construção do teste de hipótese

1. Estabeleça a hipótese nula $H_0$ e a hipótese alternativa $H_1$.

2. Defina qual estimador do parâmetro populacional $\theta$ será usado para testar $H_0$: média, desvio padrão amostral, proporção amostral, etc.
...
5. Se não pertencer à Região Crítica, não rejeitamos $H_0$; caso contrário, rejeitamos a hipótese nula $H_0$.
